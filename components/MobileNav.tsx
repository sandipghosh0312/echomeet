        ...
        <section className='w-full max-w-[264px]'>
            <Sheet>
                ...
                <SheetContent side="left" className='border-none bg-dark-1'>
                    <Link href="/" className="flex flex-col items-center w-full gap-1">
                        ...
                        <div className="flex h-[calc(100vh-72px)] w-full flex-col justify-between overflow-y-auto">
                            <SheetClose asChild>
                                <section className="flex h-full w-full flex-col gap-6 pt-16 text-white">
                                    {sidebarLinks.map((link) => {
                                        const isActive = pathname === link.route;
                                        return (
                                            <SheetClose asChild key={link.route}>
                                                <Link href={link.route} key={link.label} className={cn('flex gap-4 items-center p-4 rounded-lg w-full md:max-w-60', {
                                                    'bg-blue-1': isActive,
                                                })}>
